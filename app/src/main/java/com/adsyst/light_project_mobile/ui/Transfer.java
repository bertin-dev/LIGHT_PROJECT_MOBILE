...
public class Transfer extends AppCompatActivity {
    ...
    @OnClick(R.id.valider_transfer)
    void valider_transfer(){
        ...
        if (validator.validate()) {
            ...
            Transfer_deposite(phone_exp, phone_benef, montant);

        }

    }

    private void Transfer_deposite(String phone_exp, String phone_benef, String montant) {
        ...
        call.enqueue(new Callback<TransfertDepositResponse>() {
            @Override
            public void onResponse(@NonNull Call<TransfertDepositResponse> call, @NonNull Response<TransfertDepositResponse> response) {
                progressDialog.dismiss();
                Log.w(TAG, "success onResponse: " + response.body() );
                assert response.body() != null;
                Global.response(context, "Transfer ou deposit", response.body().getMessage(), R.drawable.ic_baseline_swap_vert_24);
            }

            @Override
            public void onFailure(@NonNull Call<TransfertDepositResponse> call, Throwable t) {
                progressDialog.dismiss();
                Log.w(TAG, "onFailure: " + t.getMessage() );
                Global.errorResponse(context, "Transfer ou deposit", t.getMessage());
            }
        });
    }
